...
document.querySelectorAll('[data-dir]').forEach(b=>{let d=b.dataset.dir;['pointerdown','touchstart'].forEach(ev=>b.addEventListener(ev,e=>{e.preventDefault();setDir(d,true)},{passive:false}));['pointerup','pointercancel','pointerleave','touchend'].forEach(ev=>b.addEventListener(ev,e=>{e.preventDefault();setDir(d,false)},{passive:false}));b.addEventListener('contextmenu',e=>e.preventDefault())});
