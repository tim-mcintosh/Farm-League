...
let state,raf=0,last=0,nextFoodId=1,mobileDirection=null,scoreSession=null,cloudRound=null;
...
function storedScore(key){const value=Number(localStorage.getItem(key));return Number.isSafeInteger(value)&&value>=0&&value<=CONFIG.maximumLegitimateScore?value:0}function loadBest(){try{return storedScore(CONFIG.bestScoreKey)}catch{return 0}}function currentScore(){return scoreSession?.getScore()||0}function awardScore(type,points,details){return scoreSession?.award(type,points,details)||false}function saveBest(candidate){if(!Number.isSafeInteger(candidate)||candidate<0||candidate>CONFIG.maximumLegitimateScore||candidate<=state.best)return;state.best=candidate;try{localStorage.setItem(CONFIG.bestScoreKey,String(state.best))}catch{}}
...
function resetRound(){cancelAnimationFrame(raf);const best=state?state.best:loadBest();state=fresh();state.best=best;scoreSession=null;nextFoodId=1;clearInput();ui.feedback.textContent='';updateHud();draw()}
async function startRound(){$('startButton').disabled=true;cloudRound=await window.FarmLeagueCloudScores.beginRound();resetRound();scoreSession=window.FarmLeagueScore.createRound({gameId:'order-rush',gameVersion:CONFIG.gameVersion,durationSeconds:CONFIG.roundSeconds,maximumLegitimateScore:CONFIG.maximumLegitimateScore,summaryStorageKey:CONFIG.roundSummaryKey,fullDurationOutcomes:['completed']});state.running=true;ui.start.classList.add('hidden');ui.results.classList.add('hidden');last=performance.now();raf=requestAnimationFrame(loop);$('startButton').disabled=false}
...
function deliverBox(){const units=total(state.order),speed=Math.round(CONFIG.scoring.maxSpeedBonus*state.orderTime/state.orderDeadline),streakBonus=Math.min(CONFIG.scoring.maxStreakBonus,state.streak*CONFIG.scoring.streakStep),points=CONFIG.scoring.correctOrder+units*CONFIG.scoring.perUnit+speed+streakBonus;awardScore('order-delivered',points,{units,speedBonus:speed,streakBonus,orderNumber:state.orderNumber});state.completed++;state.streak++;state.bestStreak=Math.max(state.bestStreak,state.streak);burst();feedback(`Delivery complete! +${points}`);sound('correct');beginTransition('success')}
function miss(){state.missed++;state.streak=0;feedback('The truck left — all boxes cleared',true);sound('missed');beginTransition('missed')}
...
function update(dt){state.elapsed+=dt;state.timeLeft=Math.max(0,CONFIG.roundSeconds-state.elapsed);if(state.timeLeft<=0){finish();return}move(dt);state.particles.forEach(p=>{p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt});state.particles=state.particles.filter(p=>p.life>0);if(state.order){state.orderTime-=dt;if(state.orderTime<=0){miss();return}if(state.orderTime<=CONFIG.warningSeconds&&!state.warning){state.warning=true;feedback('Truck leaving soon!');sound('warning')}updateConveyor(dt);handleAutomaticInteractions()}else updateTransition(dt)}
function accuracy(){const n=state.completed+state.missed;return n?Math.round(state.completed/n*100):0}function finish(){state.running=false;state.elapsed=CONFIG.roundSeconds;state.timeLeft=0;clearInput();const summary=scoreSession?.finalize({outcome:'completed',elapsedSeconds:state.elapsed,facts:{completed:state.completed,missed:state.missed,bestStreak:state.bestStreak}});const finalScore=summary?.finalScore??0,coinReward=window.FarmLeagueCoins.rewardRound(summary,{previousBest:state.best});if(summary?.valid)saveBest(finalScore);updateHud();$('finalScore').textContent=finalScore.toLocaleString();$('finalBest').textContent=state.best.toLocaleString();$('finalCompleted').textContent=state.completed;$('finalMissed').textContent=state.missed;$('finalAccuracy').textContent=`${accuracy()}%`;$('finalStreak').textContent=state.bestStreak;$('finalFarmCoins').textContent=`+${coinReward.coins}`;window.FarmLeagueCloudScores.renderSubmission(summary,cloudRound,$('cloudScoreStatus'));ui.results.classList.remove('hidden');sound('roundEnd')}
