...
function accuracy(){const n=state.completed+state.missed;return n?Math.round(state.completed/n*100):0}function finish(){state.running=false;state.elapsed=CONFIG.roundSeconds;state.timeLeft=0;clearInput();const summary=scoreSession?.finalize({outcome:'completed',elapsedSeconds:state.elapsed,facts:{completed:state.completed,missed:state.missed,bestStreak:state.bestStreak}});const finalScore=summary?.finalScore??0;if(summary?.valid)saveBest(finalScore);updateHud();$('finalScore').textContent=finalScore.toLocaleString();$('finalBest').textContent=state.best.toLocaleString();$('finalCompleted').textContent=state.completed;$('finalMissed').textContent=state.missed;$('finalAccuracy').textContent=`${accuracy()}%`;$('finalStreak').textContent=state.bestStreak;ui.results.classList.remove('hidden');sound('roundEnd')}
