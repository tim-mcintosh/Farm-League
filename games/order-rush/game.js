...
let state,raf=0,last=0,nextFoodId=1,mobileDirection=null,scoreSession=null;
...
function startRound(){resetRound();scoreSession=window.FarmLeagueScore.createRound({gameId:'order-rush',gameVersion:CONFIG.gameVersion,durationSeconds:CONFIG.roundSeconds,maximumLegitimateScore:CONFIG.maximumLegitimateScore,summaryStorageKey:CONFIG.roundSummaryKey,fullDurationOutcomes:['completed']});state.running=true;ui.start.classList.add('hidden');ui.results.classList.add('hidden');last=performance.now();raf=requestAnimationFrame(loop)}
